...
(function (factory) {
  ...
  var _saveAs = (function (view) {
    ...
    if (typeof view === "undefined" || typeof navigator !== "undefined" && /MSIE [1-9]\./.test(
        navigator.userAgent)) {
      return;
    }
    var
      ...
      if ($.isPlainObject(val)) {
        ...
        if (_ieExcel) {
          ...
          for (i = 0, ien = attrs.length; i < ien; i++) {
            var attr = val.createAttribute(attrs[i].name.replace(':',
  ...
  var excelStrings = {
    ...
    "xl/styles.xml": '<?xml version="1.0" encoding="UTF-8"?>' +
      ...
      '<u />' +
      '</font>' +
      '<font>' + // 5. font  [Comfortaa, 10, normal, white]
      '<sz val="10" />' +
      '<name val="Comfortaa" />' +
      '<color rgb="FFFFFFFF" />' +
      '</font>' +
      '<font>' + // 6. font  [Comfortaa, 9, normal, black]
      '<sz val="9" />' +
      '<name val="Comfortaa" />' +
      '</font>' +
      '</fonts>' +
      '<fills count="10">' + // <<< Fills start >>>
      '<fill>' + // 0.fill
      '<patternFill patternType="none" />' +
      '</fill>' +
      '<fill>' +
      // 1. fill Excel appears to use this as a dotted background regardless of values but
      ...
      '</fill>' +
      '<fill>' + // 6. fill
      '<patternFill patternType="solid">' + // 6. fill [darkest grey]
      '<fgColor rgb="434343" />' +
      '</patternFill>' +
      '</fill>' +
      '<fill>' + // 7. fill
      '<patternFill patternType="solid">' + // 7. fill [light grey]
      '<fgColor rgb="CCCCCC" />' +
      '</patternFill>' +
      '</fill>' +
      '<fill>' + // 8. fill
      '<patternFill patternType="solid">' + // 8. fill [lightest grey]
      '<fgColor rgb="F3F3F3" />' +
      '</patternFill>' +
      '</fill>' +
      '<fill>' + // 9. fill
      '<patternFill patternType="solid">' + // 9. fill [darker grey]
      '<fgColor rgb="999999" />' +
      '</patternFill>' +
      '</fill>' +
      '</fills>' +
      ...
      '</cellStyleXfs>' +
      '<cellXfs count="71">' + // <<< cellXfs start >>>
      ...
      // 66. style
      '<xf numFmtId="0" fontId="5" fillId="6" borderId="0" applyFont="1" applyFill="1" applyBorder="1" xfId="0" applyAlignment="1">' +
      // 67. style
      '<alignment horizontal="center" vertical="center"/>' +
      '</xf>' +
      '<xf numFmtId="0" fontId="6" fillId="7" borderId="0" applyFont="1" applyFill="1" applyBorder="1" xfId="0" applyAlignment="1">' +
      // 68. style
      '<alignment horizontal="center" vertical="center"/>' +
      '</xf>' +
      '<xf numFmtId="0" fontId="6" fillId="8" borderId="0" applyFont="1" applyFill="1" applyBorder="1" xfId="0" applyAlignment="1">' +
      // 69. style
      '<alignment horizontal="left"/>' +
      '</xf>' +
      '<xf numFmtId="0" fontId="6" fillId="9" borderId="0" applyFont="1" applyFill="1" applyBorder="1" xfId="0" applyAlignment="1">' +
      // 70. style
      '<alignment horizontal="left"/>' +
      '</xf>' +
      '<xf numFmtId="0" fontId="6" fillId="9" borderId="0" applyFont="1" applyFill="1" applyBorder="1" xfId="0" applyAlignment="1">' +
      ...
      '<alignment horizontal="center"/>' +
      '</xf>' +
      '<xf numFmtId="0" fontId="6" fillId="0" borderId="0" applyFont="1" applyFill="1" applyBorder="1" xfId="0" applyAlignment="1">' +
      // 72. style
      '<alignment horizontal="center"/>' +
      '</xf>' +
      '</cellXfs>' +
  ...
  DataTable.ext.buttons.excelHtml5 = {
    ...
    available: function () {
      return window.FileReader !== undefined && _jsZip() !== undefined && !_isDuffSafari() &&
        _serialiser;
    },
    ...
    action: function (e, dt, button, config) {
      ...
      var addRow = function (row) {
        ...
        for (var i = 0, ien = row.length; i < ien; i++) {
          ...
          for (var j = 0, jen = _excelSpecials.length; j < jen; j++) {
            ...
            if (row[i].match && !row[i].match(/^0\d+/) && row[i].match(special
                .match)) {
      ...
      if (config.autoFilter) {
        $('mergeCells', rels).before(_createNode(rels, 'autoFilter', {
          attr: {
            ref: 'A' + dataStartRow + ':' + createCellPos(data.header
              .length - 1) + dataEndRow
          }
        }));
        ...
        $('definedNames', workbook).append(_createNode(workbook, 'definedName', {
          ...
          text: _sheetname(config) + '!$A$' + dataStartRow + ':' +
            createCellPos(data.header.length - 1) + dataEndRow
        }));
      }
