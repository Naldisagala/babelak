    ...
    <div class="col-8">
        <div class="card">
            <div class="card-body">
                ...
                <br><br>
                <p><i class="fa fa-check"></i> <b>Tingkat Pemakaian</b><br><small>{{$barang->status_barang}}</small></p>
                <p><i class="fa fa-location-dot"></i> <b>Lokasi</b><br><small>{{$barang->seller->alamat->kota}}</small></p>
...
<div class="row mt-4">
    ...
    <div class="col">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title"><b><img src="{{url('/image/bangku_kecil.jpeg')}}" class="barang-img-profile" alt="" srcset=""> {{$barang->seller->nama_toko}}</b> <br><small style="font-size: 12px">{{$barang->seller->alamat->kota}}</small></h5>
                <div class="chat-area-barang" style="display: flex; flex-direction: column-reverse;padding:10px;">
                    ...
                    <form action="/tawar/1/{{$barang->seller->id}}/{{$barang->id}}/null" method="POST">
                        ...
                        <div class="input-group mb-3">
                            <input type="number" class="form-control" name="tawar" placeholder="Ketikkan tawaran" aria-label="Ketikkan tawaran" aria-describedby="basic-addon2">
                            <button style="background: #CB63A3!important" class="input-group-text" id="basic-addon2" type="submit"><i style="color:white;" class="fa fa-paper-plane"></i></button>
                        </div>
                    </form>
                    <br>
                    <div>
                        @php
                            $rekomendasi_tawar = [
                                $barang->harga - ($barang->harga * 5 / 100),
                                $barang->harga - ($barang->harga * 10 / 100),
                                $barang->harga - ($barang->harga * 15 / 100)
                            ];
                        @endphp
                        @foreach ($rekomendasi_tawar as $tawar)
                            <a style="text-decoration: none" href="/tawar/1/{{$barang->seller->id}}/{{$barang->id}}/{{$tawar}}">
                    ...
                    <div>
                        <div style="overflow-y: scroll; display: flex; flex-direction: column; max-height: 250px;">
                                ...
                                @foreach ($barang->tawar as $tawar)
                                    <div>
                                        <div class="bubble-chat">
                                            Saya menawar: <b>{{"Rp " . number_format($tawar->harga_tawar, 0, ',', '.')}}</b> <br>
                            ...
                            @else
                                    <br>
                                <span style="text-align: center; color: white; font-weight: 600">Silahkan Tawar</span>
                                <br>
                            @endif
                        </div>
                    </div>
